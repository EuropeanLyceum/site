...
const FinancialReportsPage = () => {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            setIsLoading(true);
            try {
                // Фільтруємо ТІЛЬКИ кореневі звіти (parentId=null), решту підтягне Prisma через include
                const params = new URLSearchParams({
                    page: page.toString(),
                    limit: '10',
                    parentId: 'null', // Наш API обробить це як фільтр для кореневих елементів
                    category: 'FINANCIAL', // Твоя категорія з Prisma
                });
    ...
    return (
        <Container maxWidth="lg" sx={{py: 2, mb: 1, minHeight: "450px"}}>
            ...
            {isLoading ? (
                <Box sx={{display: 'flex', justifyContent: 'center', py: 10}}><CircularProgress
                    sx={{color: '#182BA1'}}/></Box>
            ) : reports.length === 0 ? (
                <Box sx={{textAlign: 'center', py: 10}}><Typography color="text.secondary">Нічого не
                    знайдено</Typography></Box>
            ) : (
                <Stack spacing={4}>
                    {reports.map((report) => (
...
const ReportSection = ({report, l}) => (
    <Paper elevation={0} sx={{p: {xs: 2, md: 4}, borderRadius: 3, bgcolor: '#F8FAFC', border: '1px solid #E2E8F0'}}>
        <Typography variant="h4" sx={{
            fontWeight: 700, color: '#182BA1', mb: 3, fontSize: {xs: 20, md: 28},
            borderLeft: '4px solid #182BA1', pl: 2
        }}>
            {l(report.titleUk, report.titleEn)}
        </Typography>

        {/* Головні документи звіту */}
        {report.documents?.length > 0 && <DocumentList documents={report.documents} l={l}/>}

        {/* Вкладені підзвіти (SubReports) */}
        {report.subReports?.map((sub) => (
            <Accordion key={sub.id} elevation={0}
                       sx={{bgcolor: 'transparent', borderBottom: '1px solid #CBD5E1', '&:before': {display: 'none'}}}>
                <AccordionSummary expandIcon={<ExpandMoreIcon/>}>
                    <Typography sx={{fontWeight: 600, fontSize: {xs: 16, md: 18}}}>
                        {l(sub.titleUk, sub.titleEn)}
                    </Typography>
                </AccordionSummary>
                <AccordionDetails>
                    {sub.descriptionUk && (
                        <Typography sx={{mb: 2, fontSize: 14, color: '#475569'}}>
                            {l(sub.descriptionUk, sub.descriptionEn)}
                        </Typography>
                    )}
                    {/* Документи підзвіту */}
...
const DocumentList = ({documents, l}) => (
    <Stack spacing={1.5} sx={{my: 1}}>
        {documents.map((doc) => (
            <Box key={doc.id} sx={{
                display: 'flex', alignItems: 'flex-start', gap: 2, p: 1.5, borderRadius: 2,
                '&:hover': {bgcolor: 'rgba(24, 43, 161, 0.04)'}, transition: '0.2s'
            }}>
                <InsertDriveFileIcon sx={{color: '#182BA1', mt: 0.5}}/>
                <Box>
                    <Link href={doc.url} target="_blank" sx={{
                        fontWeight: 600,
                        color: '#182BA1',
                        textDecoration: 'none',

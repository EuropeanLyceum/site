...
import styles from '@/app/educational-process/project-research/project-research.module.css';
...
export default function ProjectResearchPage() {
  ...
  console.log('ProjectResearch - Current locale:', locale);
  console.log('ProjectResearch - Translation test:', t('projectResearch'));

  // Відстеження змін мови
  useEffect(() => {
    console.log('ProjectResearch - Language changed to:', locale);
  ...
  return (
    <>
      <div className={styles.projectResearchPage} lang={locale} key={`${locale}-${renderKey}`}>
        <div className={styles.intellectContent}>
          <h1 className={styles.intellectTitle}>{t("projectResearch")}</h1>
          
          <div className={styles.introText}>
            <p>{t("projectResearchIntro")}</p>
            
            <p><strong>{t("projectMethodTitle")}</strong> {t("projectMethodDefinition")}</p>
            
            <p><strong>{t("projectMethodAllows")}</strong></p>
            <ul>

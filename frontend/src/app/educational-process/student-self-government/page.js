...
export default function StudentGovernmentPage() {
  ...
  // Уніфікована функція завантаження з підтримкою пошуку та пагінації
  const fetchData = useCallback(async ({ search, page }) => {
    setLoading(true);
    try {
      const limit = 5;
      const query = new URLSearchParams({
        type: 'GOVERNMENT', // Тип контенту згідно з вашою схемою БД
  ...
  return (
      <UnifiedNewsLayout
          ...
          totalCount={total}
          isLoading={loading}
          onParamsChange={fetchData} // Передаємо функцію для обробки змін "на льоту"
      />
  );
}